...
(async()=>{
 ...
 try {
  ...
  const filterButton=page.getByRole('button',{name:'Filter documents'}); if(await filterButton.count())await filterButton.first().click(); const filter=page.getByRole('textbox',{name:'Filter documents'}); await filter.waitFor({state:'visible',timeout:90000});
  await filter.fill('Moved guide'); await sleep(2500);
  const moved=page.getByRole('list',{name:'Space documents'}).getByRole('listitem').filter({hasText:'Moved guide'}); const movedCount=await moved.count(); const movedText=await moved.allTextContents();
  await filter.fill('Republished guide'); await sleep(2500);
  const republished=page.getByRole('list',{name:'Space documents'}).getByRole('listitem').filter({hasText:'Republished guide'}); const republishCount=await republished.count(); const republishText=await republished.allTextContents();
