...
(async()=>{
 ...
 try {
  ...
  const target=`${baseUrl}/hm/${account}`; await page.goto(target,{waitUntil:'domcontentloaded',timeout:90000});
  const moved=page.getByText('Moved guide',{exact:true}); await moved.first().waitFor({state:'visible',timeout:90000}); const movedCount=await moved.count(); const movedText=await moved.allTextContents();
  const republished=page.getByText('Republished guide',{exact:true}); const republishCount=await republished.count(); const republishText=await republished.allTextContents();
  await page.screenshot({path:out+'/file-browser.png',fullPage:true});
  const result={label,sourceSha:label,surface:'web',url:target,startedAt,finishedAt:new Date().toISOString(),status:movedCount===1&&republishCount>=1?'BASELINE_NOT_REPRODUCED':'REPRODUCED',assertion:{movedGuideExpectedCount:1,movedGuideActualCount:movedCount,republishExpectedMinimum:1,republishActualCount:republishCount},movedText,republishText,issues};
