...
(async()=>{
 ...
 try {
  ...
  const lists=page.getByRole('list',{name:'Space documents'}); await lists.first().waitFor({state:'visible',timeout:90000});
  const moved=lists.getByRole('listitem').filter({hasText:'Moved guide'}); const movedCount=await moved.count(); const movedText=await moved.allTextContents();
  const republished=lists.getByRole('listitem').filter({hasText:'Republished guide'}); const republishCount=await republished.count(); const republishText=await republished.allTextContents();
